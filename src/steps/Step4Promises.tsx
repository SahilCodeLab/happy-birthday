...
import { stopAllMusic, handleBlockMusic } from "../musicController";
import flower1_1 from "../saba/flower1 (1).png";
import flower4 from "../saba/flower4.png";
import flower1_3 from "../saba/flower1 (3).png";
...
export default function Step4Promises({ onNext, onPrev }: Step4Props) {
  ...
  const paragraphLines = [
    "Ye kuch tasveerein hain...",
    "Jo aapne kabhi mujhe bheji thi. 🌸",
  ...
  useEffect(() => {
    // Stop any previous music and start the specific track for this step (block 3)
    void stopAllMusic();
    // void handleBlockMusic(3); // audio disabled for this step
  ...
  return (
    <div className="relative w-full min-h-screen flex flex-col items-center justify-between py-3 sm:py-6 px-4 overflow-hidden z-10 select-none bg-gradient-to-b from-background via-cream-dark to-background">
      ...
      <div className="absolute inset-x-0 bottom-0 flex justify-center items-end pointer-events-none z-0">
        <CandleFlame className="w-24 h-48 opacity-10 filter blur-2xl md:w-32 md:h-64" />
      </div>

      {/* Header Area (Reduced Vertical Footprint) */}
      <div className="text-center z-10 mt-0.5 sm:mt-1 shrink-0 flex flex-col items-center">
        <motion.div
        ...
        >
          ...
          {!showIntro && (
            <motion.button
              ...
              className="-mt-1 flex items-center gap-1 px-3 py-0.5 bg-white/40 backdrop-blur-sm border border-primary/20 rounded-full text-[11px] sm:text-xs text-[#7a6656] font-medium transition-all shadow-sm hover:bg-white/60 active:bg-white/70 mb-0"
            >
              <span>Read Note 📝</span>
            </motion.button>
          )}
        ...
        <AnimatePresence mode="wait">
          {showIntro ? (
            <motion.div
              ...
              transition={{ duration: 0.4, ease: "easeOut" }}
              className="z-30 w-[92vw] max-w-[360px] bg-[#FCFAF2] border-2 border-primary/20 shadow-2xl rounded-2xl p-5 sm:p-6 flex flex-col justify-between relative overflow-hidden"
            >
              {/* Corner flowers */}
              <img src={yellowFlower} alt="yellow flower" className="absolute top-2.5 left-1 w-24 h-24 pointer-events-none opacity-80" />
              <img src={whiteFlower} alt="white flower" className="absolute top-2 right-2 w-20 h-20 object-contain pointer-events-none opacity-80 z-10" />

              {/* Notebook binding top style */}
              ...
              >
                ...
                <motion.div
                  ...
                  className="p-1.5 rounded-full bg-primary/10 text-primary mb-2 shadow-inner"
                >
                  <Heart className="w-4 h-4 fill-primary/25" />
                </motion.div>

                <motion.h2
                  variants={lineVariants}
                  className="font-display text-sm sm:text-base text-primary tracking-widest uppercase mb-2 font-semibold"
                >
                  Saba... 🌸
                </motion.h2>

                <div className="space-y-2 sm:space-y-2.5 w-full my-0 sm:my-0">
                  {paragraphLines.map((line, idx) => (
                    <motion.p
                      key={idx}
                      variants={lineVariants}
                      className="font-handwriting text-base sm:text-[17px] text-[#7a6656] leading-normal sm:leading-relaxed m-0 italic font-medium"
                    >
                      {line}
                    </motion.p>
                  ))}
                ...
                <motion.div
                  variants={lineVariants}
                  className="w-full pt-3 sm:pt-4 border-t border-primary/10 mt-3 sm:mt-4"
                >
                  <Button
                    className="btn-paper w-full flex items-center justify-center gap-1.5 py-1.5 sm:py-2 px-4 text-[#7a6656] font-bold text-sm sm:text-base bg-cover bg-center bg-no-repeat transition-transform hover:scale-[1.02] active:scale-[0.98] shadow-md"
                  ...
                  >
                    {albumOpened ? "Back to Album 💖" : "Open Album 📸"}
                  </Button>
                </motion.div>
              </motion.div>
            </motion.div>
            ...
            >
                ...
                <div className={`relative w-full max-w-[95vw] sm:max-w-[420px] flex items-center justify-center ${isMobile
                  ...
                  {albumImages.map((image, index) => {
                    ...
                    const scale = isActive
                      ? (isMobile ? 1.12 : 1.05)
                      : (isMobile ? 0.78 : 0.85);
                    // Disable Y-rotation on mobile for buttery smooth GPU performance
                    ...
                    return (
                      ...
                      >
                        ...
                        >
                          ...
                          >
                            <Heart
                              className={`h-5 w-5 stroke-primary/55 transition-colors ${liked[index] ? "fill-red-500 stroke-red-500 scale-110 drop-shadow-[0_2px_8px_rgba(239,68,68,0.5)]" : "text-primary/70"
                                }`}
                            />
                          </button>
              ...
              <div className="flex gap-2 z-10 mt-2 mb-1 justify-center items-center shrink-0">
                {albumImages.map((_, index) => (
                  <button
                    key={index}
                    onClick={() => setActiveIndex(index)}
                    className={`transition-all duration-300 rounded-full ${index === activeIndex ? "w-3.5 h-1 bg-primary" : "w-1 h-1 bg-primary/30 hover:bg-primary/50"
                      }`}
                  />
                ))}
              </div>
